...
You may move a disposition to a better unit rather than accepting or rejecting it wholesale. The canonical case (AE1): a lens proposes cutting an enumerating **table** because one of its rows is duplicated by a standalone **example**. The table is load-bearing — it enumerates cases the example doesn't. Redirect: **keep the table, cut the redundant example instead.** Retarget the cut to whichever example is the *redundant duplicate* — the second illustration of a row already shown — never to the first illustration and never to the table. Record the redirect in `evidence` (e.g. "table U12 enumerates 4 cases and is load-bearing; cut retargeted to the duplicate example that merely re-illustrates the row another example already covers"). The unit IDs here are illustrative; reason from the roles (table / first example / duplicate example), not from the literal numbers. The lenses identify *that* something is redundant; you decide *which* unit absorbs the change so the load-bearing side survives.
